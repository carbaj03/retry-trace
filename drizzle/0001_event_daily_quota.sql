...
WHEN COALESCE((SELECT count FROM event_daily_quota WHERE day=substr(NEW.at,1,10)),0)>=20000
BEGIN
  SELECT RAISE(IGNORE);
END;
--> statement-breakpoint
...
BEGIN
  INSERT INTO event_daily_quota(day,count) VALUES (substr(NEW.at,1,10),1)
  ON CONFLICT(day) DO UPDATE SET count=count+1;
END;
--> statement-breakpoint
-- Seed only the current UTC day once; historical records remain untouched.
INSERT INTO event_daily_quota(day,count)
SELECT substr(at,1,10),COUNT(*) FROM events WHERE at>=date('now') GROUP BY substr(at,1,10)
